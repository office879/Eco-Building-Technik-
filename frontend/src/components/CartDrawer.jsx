...
export default function CartDrawer() {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      setStep("success");
      clear();
      toast.success(t("cart.toastOk"));
    } catch {
      toast.error(t("cart.toastErr"));
    } finally {
      setSending(false);
    }
  };
  ...
  const handleStripeCheckout = async () => {
    if (!allHavePrice) {
      toast.error(t("cart.toastNoPrice"));
      return;
    }
    setCheckingOut(true);
    try {
      ...
      const msg = err?.response?.data?.detail || t("cart.toastCheckoutErr");
      toast.error(msg);
    } finally {
    ...
    <div className="fixed inset-0 z-[60]" data-testid="cart-drawer">
      ...
      <aside className="absolute right-0 top-0 bottom-0 w-full sm:w-[480px] bg-[#0B1736] border-l border-white/10 flex flex-col">
        <div className="flex items-center justify-between p-7 border-b border-white/10">
          <div>
            <div className="eyebrow">{t("cart.inquiry")}</div>
            <div className="font-display text-2xl mt-2">
              {step === "success" ? t("cart.sent") : step === "form" ? t("cart.yourData") : `${count} ${t("cart.articles")}`}
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {step === "cart" && (
            items.length === 0 ? (
              <div className="p-12 text-center text-white/50" data-testid="cart-empty">
                ...
                {t("cart.emptyMain")}<br/>{t("cart.emptySub")}
              </div>
            ) : (
          ...
          {step === "form" && (
            <form onSubmit={handleSend} className="p-7 space-y-1" data-testid="inquiry-form">
              <div className="eyebrow mb-5">{t("cart.step2")}</div>
              <input required placeholder={t("cart.namePh")} className="input-field"
                value={form.customer_name} onChange={e=>setForm({...form, customer_name:e.target.value})}
                data-testid="form-name"/>
              <input required type="email" placeholder={t("cart.emailPh")} className="input-field"
                value={form.email} onChange={e=>setForm({...form, email:e.target.value})}
                data-testid="form-email"/>
              <input placeholder={t("cart.phonePh")} className="input-field"
                value={form.phone} onChange={e=>setForm({...form, phone:e.target.value})}
                data-testid="form-phone"/>
              <input placeholder={t("cart.companyPh")} className="input-field"
                value={form.company} onChange={e=>setForm({...form, company:e.target.value})}
                data-testid="form-company"/>
              <textarea placeholder={t("cart.messagePh")} rows={3} className="input-field resize-none"
                value={form.message} onChange={e=>setForm({...form, message:e.target.value})}
                data-testid="form-message"/>
              <div className="text-xs text-white/50 pt-5">
                {count} {t("cart.willSend")}
              </div>
            </form>
          )}
          ...
          {step === "success" && (
            <div className="p-12 text-center" data-testid="cart-success">
              ...
              <div className="font-display text-xl mb-2">{t("cart.thanks")}</div>
              <div className="text-white/60 text-sm">{t("cart.thanksSub")}</div>
            </div>
          )}
        </div>
          ...
          <div className="p-7 border-t border-white/10 space-y-3">
            {netTotal > 0 && (
              <div className="mb-3 text-xs space-y-1.5" data-testid="cart-totals">
                <div className="flex justify-between text-white/55"><span>{t("cart.netto")}</span><span>€ {netTotal.toFixed(2)}</span></div>
                <div className="flex justify-between text-white/55"><span>{t("cart.vat")}</span><span>€ {vat.toFixed(2)}</span></div>
                <div className="flex justify-between text-white pt-1.5 border-t border-white/10 font-medium tracking-wider"><span>{t("cart.total")}</span><span>€ {gross.toFixed(2)}</span></div>
                {!allHavePrice && (
                  <div className="text-[10px] text-amber-300/80 pt-1">{t("cart.onRequest")}</div>
                )}
              </div>
            )}
            <button
            ...
            >
              {checkingOut ? t("cart.preparing") : <>{t("cart.buyOnline")} <CreditCard size={14}/></>}
            </button>
            <button
            ...
            >
              {t("cart.sendInquiry")} <ArrowRight size={14}/>
            </button>
            <button onClick={clear} className="text-[11px] tracking-[0.12em] uppercase text-white/50 hover:text-white w-full">{t("cart.clearCart")}</button>
          </div>
        )}

        {step === "form" && (
          <div className="p-7 border-t border-white/10 flex gap-3">
            <button onClick={()=>setStep("cart")} className="btn-ghost flex-1 justify-center" data-testid="back-to-cart">{t("cart.back")}</button>
            <button type="submit" onClick={handleSend} disabled={sending} className="btn-primary flex-1 justify-center disabled:opacity-60" data-testid="submit-inquiry">
              {sending ? t("cart.submitting") : <>{t("cart.submit")} <ArrowRight size={14}/></>}
            </button>
          </div>
        )}
