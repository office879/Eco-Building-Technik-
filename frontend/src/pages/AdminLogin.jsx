...
export default function AdminLogin() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Willkommen zurück, Admin");
    } catch (err) {
      const msg = err?.response?.data?.detail || "Login fehlgeschlagen";
      setError(typeof msg === "string" ? msg : "Login fehlgeschlagen");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="pt-32 pb-20 min-h-screen" data-testid="admin-login-page">
      <div className="max-w-md mx-auto px-4 sm:px-6">
        <div className="eyebrow mb-6">Administration</div>
        <h1 className="font-display text-5xl md:text-6xl tracking-tight leading-[0.95]">
          Admin <span className="italic-accent">Login</span>
        </h1>
        <p className="mt-6 text-white/60 text-sm">
          Geschützter Bereich. Nur autorisierte Mitarbeiter.
        </p>

        <form onSubmit={onSubmit} className="mt-12 border border-white/10 p-8 space-y-6" data-testid="admin-login-form">
          <div>
            <label className="eyebrow text-xs mb-3 block">E-Mail</label>
          ...
          <div>
            <label className="eyebrow text-xs mb-3 block">Passwort</label>
            <div className="relative">
